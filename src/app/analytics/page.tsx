...
const CATEGORY_TAGS = {
    star: { label: 'Star', color: 'bg-green-100 text-green-700', desc: 'Populaire & Rentable' },
    plowhorse: { label: 'Plowhorse', color: 'bg-blue-100 text-blue-700', desc: 'Populaire mais peu rentable' },
    puzzle: { label: 'Puzzle', color: 'bg-amber-100 text-amber-700', desc: 'Rentable mais peu populaire' },
    dog: { label: 'Dog', color: 'bg-red-100 text-red-700', desc: 'Faible vente & Faible marge' },
};
...
export default function AnalyticsPage() {
    ...
    return (
        <div className="flex flex-col gap-10 pb-12 -m-8 p-8 bg-bg-primary min-h-screen animate-in fade-in duration-700">
            {/* Executive Header */}
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 bg-white p-10 rounded-xl border border-border shadow-sm">
                <div>
                    <h1 className="text-4xl font-serif font-semibold text-text-primary tracking-tight">Intelligence Opérationnelle</h1>
                    <div className="flex items-center gap-3 mt-4">
                        <div className="w-1.5 h-1.5 rounded-full bg-accent animate-pulse" />
                        <p className="text-text-muted font-bold uppercase tracking-[0.2em] text-[10px] flex items-center gap-2">
                            Rapport Stratégique &bull; Temps Réel
                        </p>
                    </div>
                </div>
                <div className="flex gap-4">
                    <Button variant="outline" className="h-12 px-8 rounded-lg font-bold text-[11px] uppercase tracking-widest border-border text-text-muted hover:text-text-primary transition-all">
                        Exporter Rapport
                    </Button>
                    <Button className="h-12 px-8 bg-accent hover:bg-black text-white rounded-lg font-bold text-[11px] uppercase tracking-widest shadow-xl shadow-accent/10 transition-all">
                        Décembre 2025 <ChevronDown strokeWidth={1.5} className="w-4 h-4 ml-3" />
                    </Button>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-10">
                ...
                <div className="xl:col-span-2 space-y-8">
                    <div className="bg-white p-10 rounded-xl border border-border shadow-sm">
                        ...
                        <div className="space-y-3">
                            {/* Table Headers */}
                            <div className="grid grid-cols-12 gap-6 px-8 py-5 text-[10px] font-bold text-text-muted uppercase tracking-[0.2em] border-b border-border/50 mb-4 bg-bg-tertiary/20 rounded-t-lg">
                            ...
                            <div className="space-y-3">
                                {filteredMenu.map((item, i) => {
                                    ...
                                    return (
                                        <div key={item.productId} className="grid grid-cols-12 gap-6 p-6 rounded-xl bg-white hover:bg-bg-tertiary border border-border/40 hover:border-accent/40 hover:shadow-xl transition-all duration-500 items-center group">
                                            ...
                                            <div className="col-span-2 flex justify-end">
                                                <div className={cn("px-3 py-1.5 rounded-full text-[9px] font-bold uppercase tracking-[0.15em] border border-current",
                                                    item.category === 'star' ? "text-success bg-success-soft" :
                                                        item.category === 'plowhorse' ? "text-accent bg-accent/5" :
                                                            item.category === 'puzzle' ? "text-warning bg-warning-soft" :
                ...
                <div className="space-y-10">
                    ...
                    <div className="bg-[#141414] p-10 rounded-xl text-white shadow-2xl relative overflow-hidden border border-white/5">
                        <Flame strokeWidth={1} className="absolute -right-8 -top-8 w-40 h-40 text-white/5 rotate-12" />
                        ...
                        <div className="space-y-5 relative z-10">
                            <div className="p-6 bg-white/5 border border-white/10 rounded-xl hover:bg-white/10 transition-colors cursor-pointer group">
                                <p className="text-white font-serif font-semibold text-base group-hover:text-accent transition-colors">Révision Tarifaire: Burrata</p>
                                <p className="text-white/40 text-[12px] mt-2 font-medium leading-relaxed">
                                    Catégorie "Plowhorse". Coût matière +15%. Ajustement suggéré: +1.50€ pour préserver la marge brute.
                                </p>
                            </div>
                            <div className="p-6 bg-white/5 border border-white/10 rounded-xl hover:bg-white/10 transition-colors cursor-pointer group">
                                <p className="text-white font-serif font-semibold text-base group-hover:text-accent transition-colors">Promotion Menu: Tiramisu</p>
                                <p className="text-white/40 text-[12px] mt-2 font-medium leading-relaxed">
